...
@Component({
  standalone: true,
  selector: 'app-in-planification-edit',
  imports: [
    ErrorMessagesComponent,
    INPlanificationFormComponent
  ],
  providers: [ErrorHandlerService],
  templateUrl: './in-planification-edit.component.html',
  styleUrl: './in-planification-edit.component.css'
})
export class INPlanificationEditComponent {
  @ViewChild(INPlanificationFormComponent) iNPlanificationForm!: INPlanificationFormComponent;

  id = signal<number>(1);
  iNPlanification = signal<INPlanificationEntity|null>(null)
  ...
  async init() {
    ...
    try {
      const iNPlanificationResponse = await this.iNPlanificationService.getById(this.id())
      const iNPlanificationData = iNPlanificationResponse.data as INPlanificationEntity;

      this.iNPlanification.set(iNPlanificationData);
      this.iNPlanificationForm.fill(iNPlanificationData);
  ...
  async handleFormSubmit (e: Event) {
    ...
    try {
      await this.iNPlanificationForm.update(this.id());
    } catch (error) {
      this.errorHandler.setError(error);
    } finally {
      this.isLoading.set(false);
